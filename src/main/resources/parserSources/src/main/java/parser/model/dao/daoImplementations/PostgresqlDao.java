...
public class PostgresqlDao extends AbstractDao {
    ...
    public PostgresqlDao(@Nonnull String connectionString, @Nonnull String driverClassName) throws Exception {
    ...
    protected String[] alterTableAutoincrementPrimaryKey(@Nonnull Table table) {
        ...
        scripts[1] = "CREATE SEQUENCE " + table.getIdentifier().getName() + "_SEQ";
        scripts[2] = "ALTER TABLE " + table.getName() + "\n" +
                "   ALTER COLUMN " + table.getIdentifier().getName() + " SET DEFAULT nextval(\'" + table.getIdentifier().getName() + "_SEQ" + "\')";
        return scripts;
    }
    ...
    protected String getDatabaseType(@Nonnull DataType dataType) {
        return dataType.getPostresqlType();
    }
}
